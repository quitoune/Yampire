...
class PersonnageRepository extends ServiceEntityRepository
{
    ...
    public function findElementsBySerie(int $page, int $max, array $params = array())
    {
        if ($page < 1) {
            throw new NotFoundHttpException('La page demandée n\'existe pas');
        }
        ...
        // pagination
        $dql  = "SELECT p FROM PersonnageSerie ps JOIN ps.personnage p";
        $dql .= "WHERE ps.serie = " . $params['serie_id'];
        $query = $this->createQuery($dql)->setFirstResult($first);
        $query->orderBy($params['field'], $params['order'])->setMaxResults($max);
        
        $paginator = new Paginator($query, true);
        
        // Nombre total de personnage
        $dql  = "COUNT(p.id) FROM PersonnageSerie ps JOIN ps.personnage p";
        $dql .= "WHERE ps.serie = " . $params['serie_id'];
        $nb = $this->createQuery($dql)->getSingleScalarResult();
